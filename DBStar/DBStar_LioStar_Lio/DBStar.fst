...
noextract
let mkBigStack (_:unit) : IfcDyn (unit) = ()

noextract
let startBenchmark (i:U32.t) : IfcDyn (unit) = ()

noextract
let stopBenchmark (_:unit) : IfcDyn (unit) = ()
  
noextract
let mkDB (max:U32.t) : IfcDyn (unit) = ()
  
noextract
let syncDB (_:unit) : IfcDyn (unit) = ()
...
noextract
let getMaxId (_:unit) : IfcDyn (u:U32.t{U32.v u = (getMaxId_g ())}) = 15ul // just debug for compilation
  
noextract
let getFreeId (_:unit) : IfcDyn  (u:U32.t{U32.v u > 0 /\ U32.v u <= (getMaxId_g ())}) = 1ul
  

noextract
let setById (u:U32.t{U32.v u > 0 /\ U32.v u <= (getMaxId_g ())}) (lp:labeled paper) : IfcDyn (unit) = ()
  
noextract
let getById (u:U32.t{U32.v u > 0 /\ U32.v u <= (getMaxId_g ())}) : IfcDyn (labeled paper) = admit (); (label ({id=1ul; pdf=5UL; review1=None}) LU.C)
 

noextract
let loadDB (_:unit) : IfcDyn (unit) = ()
 

noextract
let  print (i:U32.t) : IfcDyn (unit) = ()

noextract
let  printPaper (lp:labeled paper) : IfcDyn (unit) = ()


let rec fetchPapers_h (i:U32.t{U32.v i <= getMaxId_g ()}) : IfcDyn (list (labeled paper))  (decreases (U32.v i))
 = 
  match i with
  | 0ul -> []
  | _ -> (getById i) :: (fetchPapers_h (U32.sub i 1ul))

let fetchPapers (_:unit) : IfcDyn (list (labeled paper)) = 
  (fetchPapers_h (getMaxId ()))


let rec fetchFor_h (i:U32.t{U32.v i <= getMaxId_g ()}) (lbl:labelType) : IfcDyn (list (labeled paper)) (decreases (U32.v i))
=
  match i with
  | 0ul -> []
...
let save (lp:labeled paper) : IfcDyn (labeled paper)=
...
let rec filter_user (user:labelType) (l:list (labeled paper))  : IfcDyn (list (labeled paper))=
  match l with
  | [] -> []
...
let fetchPapers_for (user:labelType) :  IfcDyn  (list (labeled paper)) = 
  let lpapers  = fetchPapers () in
  let lpapers' = filter_user user lpapers in
  lpapers'


let printing (lp:labeled paper) : IfcDyn (unit) =
  printPaper lp;
  ()

let incr (lp:labeled paper) (current:labelType) : IfcDyn (paper)= 
  let p  = unlabel lp in
...
let incr_C (lp:labeled paper) : IfcDyn (paper) = 
  incr lp LU.C
  

let add (test:fixedString) (lbl:labelType) : IfcDyn (unit)=
  let j = getFreeId () in
...
let dummy lp : IfcDyn (paper)=
  {id=1ul; pdf=0UL; review1=None}
  
let example (_:unit) : IfcDyn unit  = 
   //loadDB ();
  ...
  //let l = map (fun _ -> True) dummy l in
  //let l = map (fun _ -> True) LU.E printing l in 
  //syncDB ();
  ()
   
let rec tryAlot (step:U32.t) : IfcDyn (unit) (decreases %[U32.v step])=
...
let rec initialize (step:U32.t) : IfcDyn (unit) (decreases %[U32.v step]) = 
  add 42UL LU.M;
