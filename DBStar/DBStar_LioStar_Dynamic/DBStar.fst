...
let add (test:fixedString) (lbl:labelType) : Ifc (unit)
  (requires fun c -> c.cur = LU.Bot /\ (G.reveal c.prop) lbl)
  (ensures fun c0 x c1 -> c0 == c1)
=
  let j = getFreeId () in
...
let example (_:unit) : Ifc unit
   (requires fun c -> c.prop == G.hide (fun _ -> True)  /\ c.cur = LU.Bot)
...
/\ G.reveal c.cur = LU.Bot)
  ...
  assert(let c = IFC?.get () in c.prop == G.hide (fun _ -> True)  
/\ G.reveal c.cur = LU.Bot);
...
/\ G.reveal c.cur = LU.Bot);  
  ...
  (admit();tryAlot (20000ul)); //75000000ul
  stopBenchmark ();
  //syncDB ();
  0l
